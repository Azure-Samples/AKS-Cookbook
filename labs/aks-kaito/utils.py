...
def check_vm_availability(vm_size_to_check, location, subscription_id):
    output = run(f"az vm list-skus --location {location} --subscription {subscription_id}", "VM sizes retrieved", "Failed to retrieve VM sizes")
    vm_available = False
    if output.success and output.json_data:
        for vm_size in output.json_data:
            if vm_size.get("name").lower() == vm_size_to_check.lower():
                print(vm_size)
                if vm_size.get("capacity") == "None":
                    print_warning(f"VM size '{vm_size_to_check}' doesn't have available capacity")
                    break   
                restrictions = vm_size.get("restrictions")
                for restriction in restrictions:
                    reason_code = restriction.get("reasonCode")
                    print_warning(f"VM size '{vm_size_to_check}' has restriction: {reason_code}")
                    break   
                vm_available = True
                break
    return vm_available
